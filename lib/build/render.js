...
const renderCommon = function (page, content, data) {
  ...
  if (data.page.url) {
    ...
    const folder = data.page.title
      ? `blog/${S(data.page.category).slugify().s}/${
          S(data.page.title).slugify().s
        }`
  ...
  if (Render.layouts[layoutName]) {
    ...
    Object.assign(
      data.page.g,
      extractPartialsVariables(Render.layouts[layoutName].content),
    );
    try {
...
const extract = function (file) {
  ...
  const timestamp = moment
    .utc(content.date ? content.date : file.stats.birthtime)
    .format('X');
