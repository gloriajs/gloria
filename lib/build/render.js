...
const renderCommon = function (page, content, data) {
    ...
    if (data.page.url) {
        let folder = data.page.url;
        destination.folder = `${folder}`;
        page.name = 'index.html';
    } else {
        let folder = data.page.title ?
...
const renderStyles = function (item, dest) {
    console.log(`rendering sass file: ${item.basePath}${item.name}`);
...
const renderStylus = function (item, dest) {
    console.log(`rendering stylus file: ${item.basePath}${item.name}`);
    const result = extract(item);
