...
Project.prototype.loadConfigFromYamlFile = function () {
  ...
  if (!fs.existsSync(path.normalize(`${root}/_config.yml`))) {
    $log.warn(chalk.red(`${root}/_config.yml was not found`));
    return;
  }

  const config = yaml.load(path.normalize(`${root}/_config.yml`));
  if (
    fs.existsSync(path.normalize(`${root}/themes/${config.theme}/_config.yml`))
  ) {
    this.themeConfig = yaml.load(
      path.normalize(`${root}/themes/${config.theme}/_config.yml`),
    );
  }

  this.config = Object.assign({}, this.themeConfig, config);
  return this.config;
};
...
Project.prototype.saveYAML = function (force, location) {
  ...
  fs.writeFileSync(`${location}/_config.yml`, this.getConfig(`yaml`), {
    flag: 'w',
  });
  return $q.when({});
};
...
Project.prototype.create = function (options) {
  ...
  if (
    fs.existsSync(`${_this.config.location}/_config.yml`) &&
    options.force !== true
  ) {
    return $q.reject({ error: `Project exist, won't override` });
  }
  ...
  if (!fs.existsSync(layout)) {
    return $q.reject({
      error: `Selected layout ${_this.config.layout} doesn't exist`,
    });
  }
  ...
  fs.copy(layout, location).then(function (err) {
    if (err) {
      return deferred.reject({
        error: `failed copying layout files`,
        err: err,
      });
    }
