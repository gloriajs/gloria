...
function handler(argv) {
  ...
  if (!meta[options.type]) {
    return $log.error(
      chalk.red(`Invalid type provided, please use one of:
        post|page|layout|partial|sass|css|public`),
    );
  }

  Object.assign(options, meta[options.type]);
  options.ext = path.extname(options.name)
    ? path.extname(options.name)
    : options.ext;
  options.path = `.${path.sep}${options.folder + path.sep}${options.dest}`;
  options.fullPath = `${options.path}${options.name}${options.ext}`;
  const file = createContent(options);

  fs.ensureDir(path.dirname(options.fullPath))
    .then(function () {
      if (argv.verbose) {
        $log.log(
          chalk.green(`Folder ${path.dirname(options.fullPath)} verified.`),
        );
      }
    ...
    .then(function (data, err) {
      if (argv.verbose) {
        $log.log(chalk.green(`File ${options.fullPath} created.`));
      }
    });
