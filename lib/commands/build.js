        ...
        if (file.extension === '.html') {
            ...
            fs.readFile(file.path, `utf-8`, function (err, content) {
                ...
                if (file.name === 'index.html') {
                    destination = `${dest}${file.basePath}${file.name}`;
                } else {
                    destination = `${dest}${file.basePath}${file.name.replace('.html', '')}`;
                    fs.ensureDirSync(destination);
                    console.log(destination);
                    destination = `${dest}${file.basePath}${file.name.replace('.html', '')}/index.html`;
                }
                fs.writeFile(`${destination}`, result, function (err, r) {
                    if (err) throw err;
                    console.log(`Processed html file: ${destination}`);
                });
            });
            return;
        } 
        if (file.extension === '.md') {
            ...
            fs.readFile(file.path, `utf-8`, function (err, content) {
                ...
                if (newname === 'index.html') {
                    ...
                    destination = `${dest}${file.basePath}${file.name.replace('.md', '')}`;
                    fs.ensureDirSync(destination);
                    destination = `${dest}${file.basePath}${file.name.replace('.md', '')}/index.html`;
                }                
                fs.writeFile(`${destination}`, result, function (err, r) {
                    if (err) throw err;
                    console.log(`Processed md file: ${destination}`);
                });
            });
            return;
        } 
...
const builder = {
  ...
  clear: {
      default: true,
      description: `When different to false, it will not overwrite other files in the dest folder.`
  }
}
