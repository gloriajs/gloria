...
function sortFiles (files, dest, data) {
    ...
    files.forEach(function (file, index) {
        if (typeof Render[file.extension] === 'function') {
            ...
            const destFolder = path.normalize(`${dest}/${content.destination.folder}`);
            const destination =
                path.normalize(`${destFolder}/${content.destination.file}`);
            const page = Object.assign({}, file, content.fm, {
                content: content.content,
                destination,
                data: Object.assign({ filename: file.shortPath }, content.fm),
                type: file.extension,
            });
            result.push(page);
            return;
...
function processFiles (dest, args) {
    ...
    project.styles.forEach((item) => {
        ...
        fse.writeFile(`${destination}${item.name.replace('scss', 'css')}`,
        result.css.css, (err) => {
            if (err) {
                throw err;
            }
            $log.log(`Style file ${destination}${item.name.replace('scss', 'css')} written`);
        });
    });
