...
function handler(argv) {
    ...
    if (S((argv.dest).lastIndexOf('.', 0) == 0)) {
    ...
    fse.walk(`${root}`).on('data', function (item) {
        ...
        if (argv.git === false && (item.shortPath.lastIndexOf(`${path.sep}.git`, 0) == 0) &&
            item.name !== `.gitignore`) {
            return;
        }

        if (S((item.shortPath).lastIndexOf(`${path.sep}_`, 0) == 0)) {
            ...
            if (S(item.path).include(sassFolder) && !S((item.name).lastIndexOf(`_`, 0) == 0)) {
