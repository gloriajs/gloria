// current progress is that I have to build the files
// and in the process each post or file that has a reference to
// data needs access to it
...
function sortPosts(files, dest, data) {
    ...
    posts.forEach((post, index, arr) => {
        ...
        post.data.url = `${path.sep}blog${path.sep}${post.category}${path.sep}` +
            (post.url ? post.url : S(post.data.title).slugify().s);
...
// I'm currently here I need to render the data in the pages/posts before they
// get rendered
function processFiles(dest, args) {
    ...
    project.stylus.forEach((item) => {
        ...
        fse.writeFile(`${dest}${path.sep}sass${path.sep}${item.name.replace('.styl', 'css')}`,
        result.css.css, (err) => {
            if (err) throw err;
            console.log(`file ${root}${path.sep}sass${path.sep}${item.name} written`);
        });
...
function handler(argv) {
    ...
    project.styles = [];
    project.stylus = [];
    // this is an element for each json file
    project.jsondata = [];

    console.log(`starting to walk`);
    // item is an object with all the data below
