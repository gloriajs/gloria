...
function handler(argv) {
  if (argv.source !== 'jekyll') {
    return $log.warn(chalk.red(`Sorry, only jekyll is supported now`));
  }
  ...
  if (argv.exporto.lastIndexOf(`..`, 0) === 0) {
    $log.log(`Copying files to destination folder ${root}/${argv.exporto}`);
    fse.copySync(root, dest);
  }

  fse.walk(`${root}`).on('data', (item) => {
    if (S(item.path).include(dest)) {
      return;
    }
    ...
    if (
      item.shortPath.lastIndexOf(`/.git`, 0) === 0 &&
      item.name !== `.gitignore`
    ) {
      return;
    }
    ...
    if ([`.html`, `.md`].indexOf(item.extension) !== -1) {
      $log.log(`migrating file`, item.path);
      ...
      $log.log(`Migrated file `, item.path);
    }
  });
}
