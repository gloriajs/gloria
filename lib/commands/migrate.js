...
function handler(argv) {
    if (argv.source !== 'jekyll') {
        return console.warn(chalk.red(`Sorry, only jekyll suported by now`));
    }
    ...
    if (S(argv.exporto).startsWith(`..`)) {
        console.log(`Copying files to destinatio folder ${root}/${argv.exporto}`);
    ...
    fse.walk(`${root}`).on('data', (item) => {
        ...
        if (item.shortPath.startsWith(`/.git`) && item.name !== `.gitignore`) {
            return;
        }

        if (item.shortPath.startsWith(`/_site`)) {
            return;
        }
