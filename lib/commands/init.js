...
function handler (argv) {
    ...
    project.config.name = options.name || prompt(`Enter short name for the blog: [${options.name}]`, options.name);
    project.config.location = options.location || prompt(`Enter the folder on which you want to create the site if different to the name, [${options.location}]`, options.location);
    project.config.location = options.location ? options.location : `${S(options.name).slugify().s}`;
    ...
    project.config.longname = options.longname || prompt(`Enter the longname to use in your site's title: [${options.name}]`, options.name);
    project.config.description = options.description ||  prompt(`Enter the description to use in your site:`);

    console.log(`\n${chalk.bold.cyan('Site will be created with the following information')}`);
    console.log(project.getConfig('yaml'));

    let confirm = prompt(`Looks good?: [Yn]`, `Y`);
    if ( confirm === `y` || confirm === `Y`) {
       const location = `${process.cwd()}/${options.location}`;
       if(fs.existsSync(`${location}/_config.yml`)) {
            confirm = prompt(chalk.bgRed(`It looks like a site already exists on that folder, do you want to overwrite?: [yN]`), `N`);
            if ( confirm !== `y` && confirm !== `Y`) {
       ...
       project.create({force: true}).then(function () {
			console.log(chalk.green(`Saved!, cd into the directory '${project.config.location}' and run 'gloria serve' to get started.`));
	   }, function (err) {
...
const builder = {
  ...
  layout: {
      default: `default`,
      description: `Ignore this option, only default layout available currently`
  }
}
