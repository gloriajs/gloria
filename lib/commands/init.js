// using variable name $log, because most methods are named the same,
// so even plugin in a diff library (if we decide to) is easier
...
const optionsBuilder = function (argv) {
  ...
  if (!fs.existsSync(`${layoutsDir}${path.sep}${argv.layout}`)) {
    $log.log(`${argv.layout} doesn't exist, setting layout to default.`);
...
const inputNameAndLocation = function (argv, project, options) {
  if (argv.interactive) {
    project.config.name =
      options.name || prompt(`Enter short name for the blog: `, options.name);
    project.config.location =
      options.location ||
      prompt(
        `Enter a folder name if different from [${options.name}]:`,
        options.location,
      );
  ...
  if (!project.config.name || !project.config.location) {
    $log.warn(chalk.red(`A name and location are required`));
    return false;
  }
...
const inputRest = function (argv, project, options) {
  if (argv.interactive) {
    ...
    project.config.longname =
      options.longname ||
      prompt(
        `Enter the longname to use in your site's title: [${options.name}]`,
        options.name,
      );
...
const showSiteInfo = function (project) {
  $log.log(
    `\n${chalk.bold.cyan(
      'Site will be created with the following information',
    )}`,
  );
  $log.log(project.getConfig('yaml'));
};
...
const fileLocationNotAvailable = function (argv, options) {
  ...
  if (
    fs.existsSync(`${location}${path.sep}_config.yml`) &&
    argv.force !== true
  ) {
    confirm = prompt(
      chalk.bgRed(`It looks like a site already
             exists on that folder,
            do you want to overwrite?: [yN]`),
      `N`,
    );
    if (confirm !== `y` && confirm !== `Y`) {
      $log.log(chalk.green(`Aborting!`));
      return true;
    } else {
      return false;
    }
...
const createSite = function (project) {
  project.create({ force: true }).then(
    ...
    (err) => {
      $log.log(chalk.red(`Error creating project!`), err);
    },
  );
};
...
const handler = function (argv) {
  ...
  $log.log(
    `Creating new gloria site.\n` +
      (options.name
        ? `named: ${options.name}, in folder: ./${options.name}`
        : ``),
  );
  $log.log(`Please complete the following information to continue:`);
